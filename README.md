...
| 거래처 (CRM) | 사업자번호 자동완성, 신용등급(A/B/C/D), 거래한도, 담당자/연락처, 메모 히스토리, 거래 타임라인 |
| 반복 전표 | 템플릿 등록, 일괄 생성 |
| 자동 전표 규칙 | 거래처/금액/키워드 조건 매칭, 규칙 우선 → AI fallback |
| 전자결재 | 다단계 승인 워크플로우, 최종 승인 시 자동 확정(POSTED), 이메일/슬랙/카카오 알림 |
...
| 거래처별 현황 | 매출 TOP10 차트, 매출 vs 매입 비교 차트, 거래처별 집계 보고서 |
...
| 다국어 | 한국어/영어 전환 |
| 다크모드 | 라이트/다크/시스템 테마 |
| 접근성 (WAI-ARIA) | aria-label, role, scope, aria-expanded, 스크린리더 지원 |
...
| 전표 승인 알림 | 결재 요청/승인/반려 시 이메일 + 슬랙 웹훅 + 카카오 웹훅 자동 발송 |
...
영수증 OCR → DRAFT 전표 생성
                ↓
        결재 요청 (PENDING_APPROVAL)
                ↓
        결재 승인 → 자동 POSTED (보고서 즉시 반영)
                ↓
        시산표 / 손익계산서 / 재무상태표
                ↓
        월 마감 (DRAFT/PENDING 전표 있으면 차단)
                ↓
        결산 이월 (자산/부채/자본 이월 + 손익 마감)
```
...
| GET | `/vendors/:id/detail` | 거래처 CRM 상세 (메모/거래/계산서) |
| POST | `/vendors/:id/memos` | 거래처 메모 추가 |
| GET | `/bom/assemblies` | 조립품 목록 |
| GET | `/bom/:parentId` | BOM 조회 (부품 구성) |
| POST | `/bom/:parentId/items` | BOM 부품 추가 |
...
| 거래처 CRM | 별도 관리 | 신용등급/한도 + 담당자/메모 히스토리/거래 타임라인 |
| BOM 관리 | 별도 모듈 (고가) | 간이 BOM (부품 구성표, 자재소요량 계산, 원가 집계) |
...
| 데이터 백업 | 별도 관리 | JSON export/import, 관리자 전용 |
